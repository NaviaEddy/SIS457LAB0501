...
void ANaveAereaJugador::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction(TEXT("VerEstado"), IE_Pressed, this, &ANaveAereaJugador::Estado);
	PlayerInputComponent->BindAction(TEXT("ConsumirCapsula"), IE_Pressed, this, &ANaveAereaJugador::ConsumirCapsula);
...
void ANaveAereaJugador::NotifyHit(class UPrimitiveComponent* MyComp, AActor* Other, class UPrimitiveComponent* OtherComp, bool bSelfMoved, FVector HitLocation, FVector HitNormal, FVector NormalImpulse, const FHitResult& Hit){
	AInventoryActor* Item = Cast<AInventoryActor>(Other);
	if (Item != nullptr)
	{
...
void ANaveAereaJugador::Estado()
{
	GEngine->AddOnScreenDebugMessage(-1, 6.f, FColor::Green, FString("Velocidad: ") + FString::SanitizeFloat(MoveSpeed));
	GEngine->AddOnScreenDebugMessage(-1, 6.f, FColor::Green, FString::Printf(TEXT("Vidas: %d"), NumVidas));
	GEngine->AddOnScreenDebugMessage(-1, 6.f, FColor::Green, FString::Printf(TEXT("Armamento: %d"), bCanFire));

}

void ANaveAereaJugador::ConsumirCapsula()
{
	if (ShipInventory->CurrentInventory.Num() != 0) {

		AInventoryActor* a = ShipInventory->CurrentInventory.Pop();
		MoveSpeed *= a->Velocidad;
		NumVidas += a->Vidas;
		if (a->Armamento == 2)
			bCanFire = true;
		else if (a->Armamento == 1)
			bCanFire = false;
		ShipInventory->RemoveFromInventory(a);
		a->Destroy();
	}
}
